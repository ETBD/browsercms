...
**Exit criteria:** all **10** live in [`phase-0-baseline-and-ci.md` § Exit criteria](phase-0-baseline-and-ci.md#exit-criteria), not in this file. This plan references them by number throughout.
...
|---|---|---|---|
| **F1** | **`bundle exec rake` — the documented default command, and Travis's `script` — fails immediately.** `ci:test` → `db:install` → `db:migrate` boots the dummy app in `development`, and `test/dummy/config/database.yml` defines **only** `test`. | `bundle exec rake db:migrate` → `ActiveRecord::AdapterNotSpecified: 'development' database is not configured. Available: ["test"]`. With `RAILS_ENV=test` it boots fine. | Every command in this phase is `RAILS_ENV=test bundle exec rake …`. Fixing it properly is a decision — see [D1](#d1-rails_env). It also means `.travis.yml` could not have been green; treat the Travis config as evidence of *intent*, not of a working build. |
...
- **`bundle exec rake`, no `RAILS_ENV` prefix**, because the job-level `env:` block sets it. If [D1](#d1-rails_env) resolves toward fixing `database.yml` instead, drop the env var here.
...
| `test/assumptions_test.rb` | Fails — asserts an empty DB, `db:install` seeded one (F5) | Stage F / [D3](#d3-testassumptions_testrb) |
