...
**One of the ten has since been closed.** Optimistic locking — the worst of them, and the one Phase 4's [D6](phase-4-implementation-plan.md) explicitly escalated rather than decided — was fixed by **CMS-435**: [cms-435-optimistic-locking.md](cms-435-optimistic-locking.md). It is worth reading as a worked example of what a characterization test is *for*. Phase 4 pinned the defect as one problem; it was three, and the two Phase 4 could not see would each have been enough on their own to leave conflicts invisible. The test that pinned it is now deleted, which is what was supposed to happen.
