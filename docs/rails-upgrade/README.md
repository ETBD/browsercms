...
| **2** | [Harness migration](phase-2-harness-migration.md) | Make the test suite capable of running on Rails 5, while still on 4.2 | 🔴 **Yes** — two of the four suites do not load on Rails 5 today | 📋 **Planned** — [plan](phase-2-implementation-plan.md) · re-scoped, see below |
...
**Phase 2 has been re-scoped, and the [plan](phase-2-implementation-plan.md) is the current statement of it.** Phase 0 established that no Capybara driver is ever selected (zero `@javascript` tags, both assignments commented out), so its Poltergeist migration has nothing to migrate. Phase 1's [pre-flight scan](phase-1-implementation-plan.md#1-pre-flight-findings) further shows most of the gems it plans to move carry no Rails 5 cap at all — making them modernisation by choice, not by force. The plan's own [pre-flight measurement](phase-2-implementation-plan.md#1-pre-flight-findings) then ran all four suites on Rails 5 for the first time and found that **mocha, factory_girl, `minitest/unit` and the positional controller args are not Rails 5.0 blockers either** — while two things the phase document does not mention are: `silence_stream` (removed at 5.0) stops cucumber loading at all, and `ckeditor_rails` 4.3.4 dispatches its Railtie on `case ::Rails.version` with no Rails 5 branch, which accounts for 27 of 33 functional errors and the first 131 cucumber failures. The phase is less typing and more debugging than the document implies.
